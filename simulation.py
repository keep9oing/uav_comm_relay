...
# plot gcs with blue star
ax.plot(P[0], P[1], marker='*', color='blue', markersize=10)
# add annotation
ax.annotate('GCS', xy=(P[0], P[1]), xytext=(P[0]+1, P[1]+1))

# plot NFZ with red circle dashed line
for i in range(len(Pn)):
    circle = plt.Circle((Pn[i][0], Pn[i][1]), rho_s, color='red', fill=False, linestyle='--')
    ax.add_artist(circle)
    # add annotation
    ax.annotate('NFZ', xy=(Pn[i][0], Pn[i][1]), xytext=(Pn[i][0]+1, Pn[i][1]+1))
